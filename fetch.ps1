...
# for the JNI build. macOS/Linux: fetch.sh.
#
...
# ---- normalize into deps\current (what the JNI build points at) ----------
# A stable directory name keeps the native build flags platform-
# independent. corvid.dll is loaded explicitly by the Kotlin loader
# (System.load of the absolute path) before the JNI shim, so no PATH
# games are needed at runtime.
...
Copy-Item (Join-Path $Extracted "corvid.dll.lib") $Cur
